...
impl Inventory for StoreInventory {
    ...
    async fn add(
        ...
        // validate SKU, verify that item is present and not empty
        ...
        // if item is already present don't allow the duplicate
    ...
    async fn remove(
        ...
        let msg = match map.remove(&identifier.sku) {
            Some(_) => "succes: item was removed",
    ...
    async fn update_quantity(
        ...
        // retrieve the stockk mutable so we can update the quantity
        let mut map = self.inventory.lock().await;
